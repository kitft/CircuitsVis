import { tensor, reverse, topk as tfTopk } from "@tensorflow/tfjs";
const text = `
...
function createRandom3DActivationMatrix(shape) {
    return Array.from(Array(shape[0]), () => Array.from(Array(shape[1]), () => Array.from(Array(shape[2]), () => Math.random())));
}
const numLayers = 2;
const numSVDDirs = 30;
const k = 5;
// [samples x tokens]
export const mockTokens = chunkText(text.split(/(?=\s)/));
// [samples x layers x neurons x tokens]
const mockActivations = mockTokens.map((tokens) => {
    return tensor(createRandom3DActivationMatrix([numLayers, numSVDDirs, tokens.length]));
});
// All have shape [samples x layers x k x neurons]
export const topkVals = [];
export const topkIdxs = [];
export const bottomkVals = [];
export const bottomkIdxs = [];
for (let sampleNum = 0; sampleNum < mockActivations.length; sampleNum += 1) {
    const { values: sampleTopkValsRaw, indices: sampleTopkIdxsRaw } = tfTopk(mockActivations[sampleNum], k, true); // [layers x neurons x k]
    const { values: sampleBottomkValsRaw, indices: sampleBottomkIdxsRaw } = tfTopk(mockActivations[sampleNum].mul(-1), k, true); // [layers x neurons x k]
    // Append the sample TopkValsRaw to the topkVals array
    topkVals.push(sampleTopkValsRaw.transpose([0, 2, 1]).arraySync()); // [layers x k x neurons]
    topkIdxs.push(sampleTopkIdxsRaw.transpose([0, 2, 1]).arraySync()); // [layers x k x neurons]
    bottomkVals.push(reverse(sampleBottomkValsRaw.mul(-1), -1)
        .transpose([0, 2, 1])
        .arraySync()); // [layers x k x neurons]
    bottomkIdxs.push(reverse(sampleBottomkIdxsRaw, -1)
        .transpose([0, 2, 1])
        .arraySync()); // [layers x k x neurons]
}
export const objType = "SVD Direction";
export const layerLabels = ["10", "12"];
