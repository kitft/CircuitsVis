...
const mockTokensFlat = chunkText(text.split(/(?=\s)/));
const numLayers = 2;
const numNeurons = 2;
const numK = 3;
...
for (let l = 0; l < numLayers; l += 1) {
    ...
    for (let n = 0; n < numNeurons; n += 1) {
        ...
        for (let k = 0; k < numK; k += 1) {
            ...
            neuronActivations.push(Array.from(Array(mockTokensFlat[sampleIdx].length), () => Math.random()));
            sampleIdx += 1;
        }
...
export const neuronLabels = ["3", "42"];
